...
def name_from_path(file_path):
    ...
    file_name = "_".join(path_parts[path_parts.index(args.files) + 1 :]).replace(
        "/", "_"
    )
    return file_name
...
def remove_blobs(file_path, isPath=True):
    if args.verbose:
        print(
            f"Removing blobs (from contianer {args.container}) for '{file_path or '<all>'}'"
        )
    blob_service = BlobServiceClient(
        account_url=f"https://{args.storageaccount}.blob.core.windows.net",
        credential=storage_creds,
...
def remove_blobs_docs(file_path, isPath=True):
    if args.verbose:
        print(f"Removing blobs (from container {args.containerdocs}) for '{file_path}'")
    blob_service = BlobServiceClient(
        account_url=f"https://{args.storageaccount}.blob.core.windows.net",
        credential=storage_creds,
    )
...
def index_sections(file, sections):
    if args.verbose:
        print(f"Indexing sections from '{file}' into search index '{args.index}'")
...
def remove_from_index(file_path, isPath=True):
    if args.verbose:
        print(
            f"Removing sections from '{file_path or '<all>'}' from search index '{args.index}'"
        )
    search_client = SearchClient(
        endpoint=f"https://{args.searchservice}.search.windows.net/",
        index_name=args.index,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: prepdocs.py 'data' --storageaccount myaccount  --container mycontainer   \
           --searchservice mysearch --index myindex -v",
    )
    ...
    parser.add_argument(
        "--containerdocs", help="Azure Blob Storage container name for full docs"
    )
    parser.add_argument(
        "--storagekey",
        required=False,
        help="Optional. Use this Azure Blob Storage account key instead of the current user  \
              identity to login (use az login to set current user for Azure)",
    )
    parser.add_argument(
        "--tenantid",
        required=False,
    ...
    parser.add_argument(
        "--openaikey",
        required=False,
        help="Optional. Use this Azure OpenAI account key instead of the current user identity to login  \
            (use az login to set current user for Azure)",
    )
    parser.add_argument(
        "--index",
        help="Name of the Azure Cognitive Search index where content should be indexed  \
             (will be created if it doesn't exist)",
    )
    parser.add_argument(
        "--searchkey",
        required=False,
        help="Optional. Use this Azure Cognitive Search account key instead of the current user identity to login  \
              (use az login to set current user for Azure)",
    )
    parser.add_argument(
        "--remove",
    ...
    parser.add_argument(
        ...
        help="Use PyPdf local PDF parser (supports only digital PDFs) instead of Azure Form Recognizer service  \
            to extract text, tables and layout from the documents",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--formrecognizerkey",
        required=False,
        help="Optional. Use this Azure Form Recognizer account key instead of the current user identity  \
            to login (use az login to set current user for Azure)",
    )
    ...
    if DATA_CONVERT:
        ...
        for root, dirs, files in os.walk(args.files2convert):
            for file in files:
                ...
                file_path = os.path.join(root, file)
                target = (
                    file_path.replace("data2convert/", "data/").rsplit(".", 1)[0]
                    + ".pdf"
                )
                ...
                # handle pictures
                    ...
                    thecanvas.drawImage(
                        file_path, x, y, width=img_width, height=img_height
                    )
    ...
    if args.removeall:
        ...
        for root, dirs, files in os.walk(args.files):
            for file in files:
                ...
                local_hashmap[name_from_path(file_path)] = [
                    file_path,

...
class OpenAIClient:
    ...
    async def chat_completion(self, system_message: str, message_text: str, openai_max_message_length=1000):
        """
        Die chat_completion Methode ist eine asynchrone Methode, die dazu dient, eine Chat-Komplettierung mit der OpenAI API durchzuführen.

        Eingabeparameter:

        system_message: Eine Systemnachricht, die an die OpenAI API gesendet wird.
        message_text: Der Text der Nachricht, die an die OpenAI API gesendet wird.
        openai_max_message_length: Die maximale Länge der Nachricht, die an die OpenAI API gesendet wird. 
                                   Der Standardwert ist 1000.
        
        Die Methode beginnt mit der Erstellung einer Liste von Nachrichten, die an die OpenAI API gesendet werden. 
        Jede Nachricht ist ein Wörterbuch mit den Schlüsseln "role" und "content". Die Rolle kann "system" oder 
        "user" sein und der Inhalt ist der Text der Nachricht.

        Die Methode versucht dann, eine Chat-Komplettierung mit der OpenAI API zu erstellen. 
        Sie verwendet das Modell, die Nachrichten und verschiedene andere Parameter, um die Anfrage zu konfigurieren. 
        Die Methode wartet auf die Antwort von der OpenAI API und gibt den Inhalt der ersten Wahl in der Antwort zurück.

        Wenn während des Aufrufs der OpenAI API eine Ausnahme auftritt, fängt die Methode diese ab. 
        Wenn die Ausnahme auf eine überschrittene Kontextlänge hinweist, loggt die Methode eine Warnung, 
        kürzt den Text der Nachricht auf die maximale Länge und versucht, die Chat-Komplettierung erneut 
        zu erstellen. Wenn die Ausnahme nicht auf eine überschrittene Kontextlänge hinweist, loggt die
        Methode einen kritischen Fehler und wirft die Ausnahme erneut.
        """
        messages = [

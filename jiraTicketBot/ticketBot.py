...
from MateClient import MateClient
import ast
...
mate_label_system_messages = (
    ast.literal_eval(os.environ.get("OPENAI_CATEGORIZE_MESSAGES")) or []
)
...
openai_max_message_length = os.environ.get("OPENAI_MAX_MESSAGE_LENGTH") or 1000
...
mate_filter_label_mapping = (
    ast.literal_eval(os.environ.get("MATE_CATEGORY_FILTER_TO_PROMPT_LABELS")) or {}
)
jira_transition_name_if_unable_to_assist = os.environ.get(
    "JIRA_TRANSITION_NAME_IF_UNABLE_TO_ASSIST"
)
...
async def categorize_content(system_messages=None, message_text=None, openai_max_message_length=1000):
    """
    ...
    system_messages             : A list of system messages to be categorized. The default value is None.
    ...
    for message in system_messages:
        openai_category_response = await openai_client.chat_completion(
            system_message=message, message_text=message_text, 
            openai_max_message_length=openai_max_message_length
        )
        if category is not None:
            category = category + "_" + openai_category_response
        else:
...
issue_contains_enough_detailed_information_msg = 'Answer me with "yes" or "no" if the following request is detailed enough to give an answer:'
...
async def main():
    ...
    jira_client = JiraClient(
        server=jira_server, username=jira_username, token=jira_token
    )
    ...
    for issue in issues:
        ...
        if jira_categorization_field is not None:
            ...
            for attr in attrs:
                if categorization_input is not None:
                    ...
                    logging.debug(
                         f"Attribute {attr} of issue {issue.key} is {categorization_input}"
                    )
                else:
                    break
        if jira_client.get_last_comment(issue.key) is not None:
            ...
            logging.debug(
                f"Categorization input for issue {issue.key} is {categorization_input}"
            )
        else:
            description = str(jira_client.read_ticket(issue.key)[1])
        ...
        openai_summarized_issue = await openai_client.chat_completion(
            system_message=summarize_issue_msg, message_text=categorization_input,
            openai_max_message_length=openai_max_message_length
        )
        logging.debug(
            f"Summarized issue for issue {issue.key} is {openai_summarized_issue}"
        )
        if len(mate_label_system_messages) > 0 and len(mate_label_system_messages) < 11:
            (ai_search_category_filter, categories) = await categorize_content(
                mate_label_system_messages, openai_summarized_issue
            )
            logging.info(f"Mate categories are {categories}")
            mate_category_filter = next(
                (
                    key
                    for key, values in mate_filter_label_mapping.items()
                    if ai_search_category_filter in values
                ),
                None,
            )
        ...
        if mate_category_filter is None or mate_category_filter not in mate_categories:
            ...
            logging.debug(
                f"Excluded Categories for issue {issue.key} are {excluded_categories}"
            )
        if categories not in excluded_categories:
            logging.info(
                f"Try tio get answer by Mate for {issue.key} with categories {categories}"
            )
            mate_response = json.loads(
                mate_client.get_answer(categorization_input, overrides)
            )
            ...
            logging.debug(
                f"Mate response evaluation for issue {issue.key} is {mate_response_evaluation}"
            )
            sources = ""
            logging.debug(
                f"data points for issue {issue.key} are {mate_response['data_points']}"
            )
            ...
            if "Unable to Assist" in mate_response_evaluation:
                ...
                issue_contains_enough_detailed_information = (
...
{'='*50}"""
                    ...
                    logging.info(
                        f"Enough detailed information for issue {issue.key} - transitioning to an agent"
                    )
...
{'='*50}"""
                ...
                logging.info(
                    f"Able to assist for issue {issue.key} - transitioning to customer"
                )
                issue_response_text = f"""{'='*50}
Issue:{issue.key}\n\n*Mate understood:*
{openai_summarized_issue}\n*Mate category:*
...
{'='*50}"""
            if SIMULATE:
                logging.info(
                    f"Simulating issue {issue.key} - not adding comment to the issue or transitioning issue"
                )
                ...
                if mate_response_evaluation.replace('"', "") == "Able to Assist":
                    for data_point in mate_response["data_points"]:
                        ...
                        if data_point["docName"].endswith(".pdf"):
                            ...
                            logging.info(
                                f"Downloading PDF for issue {issue.key} from {path}"
                            )
                            pdf = mate_client.download_pdf(path)
                            # Create a temporary directory
                            logging.debug(
                                f"Creating temporary file for issue {issue.key}"
                            )
                            with tempfile.NamedTemporaryFile(
                                ...
                                temp_file.write(pdf)
                            logging.debug(
                                f"Renaming temporary file for issue {issue.key}"
                            )
                            os.rename(
                                temp_file.name,
                            ...
                            logging.info(
                                f"Attaching PDF {attachment} for issue {issue.key}"
                            )
                ...
                if not transition_to_an_agent:
                    logging.info(
                        f"Getting transition id to answer the customer for issue {issue.key}"
                    )
                    transition_id = jira_client.get_transition_id(
                        possible_transitions, jira_transition_name
                    )
                else:
                    logging.info(
                        f"Getting transition id to transfer it to an agent for issue {issue.key}"
                    )
                    transition_id = jira_client.get_transition_id(
                ...
                try:
                    logging.info(
                        f"Transitioning issue {issue.key} to transition id {transition_id}"
                    )
                    jira_client.transition_issue(issue.key, transition_id)
                except Exception:
                    logging.warning(
                        f"Transitioning issue {issue.key} failed, because it may transitioned already automatically -- please check the issue in Jira."
                    )
    await openai_client.close()

...
class MateClient:
    ...
    def get_token(self):
        ...
        try:
            if not result:
                logging.info(
                    f"No suitable token exists in cache. Let's get a new one from Azure AD."
                )
    ...
    def get_mate_categories(self):
        ...
        if token:
            ...
            response = requests.get(
                str(self.config["mate_backend_url"]) + "/categories", headers=headers
            )
    ...
    def get_answer(
        ...
        if token:
            ...
            response = requests.post(
                str(self.config["mate_backend_url"]) + "/chat", headers=headers, json=message
            )
            return response.text
        else:
            return None
    ...
    def download_pdf(self, path):
        ...
        if token:
            headers = {"Authorization": "Bearer " + token}
            response = requests.get(
                str(self.config["mate_backend_url"]) + "/content" + path, headers=headers
            )

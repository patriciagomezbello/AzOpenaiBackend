...
mate_label_system_messages = ast.literal_eval(os.environ.get("OPENAI_CATEGORIZE_MESSAGES")) or []
...
async def main():
    ...
    for issue in issues:
        ...
        if len(mate_label_system_messages) > 0 and len(mate_label_system_messages) < 11:
            (ai_search_category_filter, categories) = await categorize_content(mate_label_system_messages, openai_summarized_issue)
        ...
        if categories not in excluded_categories:
            logging.info(f"Try tio get answer by Mate for {issue.key} with categories {categories}")
            mate_response = json.loads(mate_client.get_answer(categorization_input, overrides))
            ...
            if "Unable to Assist" in mate_response_evaluation:
                ...
                if issue_contains_enough_detailed_information == "no":
                    logging.info(f"Not enough detailed information for issue {issue.key} - asking customer for more information")
                    issue_response_text = (
                        issue_response_text
                    ) = f"""{'='*50}
Issue:{issue.key}\n\n*Mate understood:*
...
{'='*50}"""
            if SIMULATE:
                ...
                if not transition_to_an_agent:
                    ...
                    transition_id = jira_client.get_transition_id(possible_transitions, jira_transition_name_if_unable_to_assist)
                try:

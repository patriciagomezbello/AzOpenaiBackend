...
class ChatData:
    ...
    def to_dict(self) -> dict[str, str]:
        ...
        return {
            ...
            "injected_prompt": self.injected_instructions if self.injected_instructions is not None else "",
        }
...
class Model:
    ...
    # TOKEN_MAPPING maps model names to token limits.
    ...
    # AZURE_OPENAI lists all the supported OpenAI model names for Azure.
    AZURE_OPENAI: list[str] = ["gpt-35-turbo", "gpt-35-turbo-16k", "gpt-4", "gpt-4-32k"]
    ...
    def is_valid(self) -> bool:
        ...
        return self.value in self.AZURE_OPENAI

    def token_limit(self) -> int:
        """token_limit returns the token limit for the model."""
        return self.TOKEN_MAPPING[self.value]


def parse_model(model: Model) -> str:
    """parse_model returns the associated openai model string for the given model.
